package priority_queue_min_min
...
type PriorityQueue []*domain.MinMinItem
...
func (pq PriorityQueue) Less(i, j int) bool {
	...
	return pq[i].UserPriority > pq[j].UserPriority
}
...
func (pq *PriorityQueue) Update(item *domain.MinMinItem, Name string, duration, initialDuration time.Duration, priority int) {
	item.Name = Name
	item.TaskDuration = duration
	item.InitialTaskDuration = initialDuration
	item.UserPriority = priority
